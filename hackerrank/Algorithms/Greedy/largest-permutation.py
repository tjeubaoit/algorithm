...
def largestPermutation(k, arr):
    n = len(arr)
    if k >= n:
        return [i for i in range(len(arr), 0, -1)]
    d = [0] * (n + 1)
    for i in range(n):
        d[arr[i]] = i
    for i in range(n):
        # Ignore values whose index is already optimal
        if arr[i] != n - i:
            k -= 1
            if k == 0:
                break
